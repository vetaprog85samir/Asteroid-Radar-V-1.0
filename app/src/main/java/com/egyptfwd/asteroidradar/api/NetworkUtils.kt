...
import com.egyptfwd.asteroidradar.utils.Constants
...
fun parseAsteroidsJsonResult(jsonResult: JSONObject): ArrayList<AsteroidEntity> {
    ...
    val asteroidModelList = ArrayList<AsteroidEntity>()
    ...
    for (formattedDate in nextSevenDaysFormattedDates) {
        if (nearEarthObjectsJson.has(formattedDate)) {
            ...
            for (i in 0 until dateAsteroidJsonArray.length()) {
                ...
                val asteroidModel = AsteroidEntity(id, codename, formattedDate, absoluteMagnitude,
